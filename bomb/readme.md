...
实验提供c代码***bomb.c***和可执行文件***bomb***（由bomb.c生成），在bomb.c中有六个“炸弹”，每个炸弹实际上就是需要我们输入参数，只有输入正确的参数才能进入下一步中执行，只有将6个炸弹都解开，才算通过。在bomb.c中只有代码整体框架，而没有更具体的代码，需要我们使用工具将可执行文件bomb反汇编成汇编代码，然后解读出答案。
